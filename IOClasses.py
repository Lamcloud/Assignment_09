            ...
            with open(file_name_Track, 'w') as file:
                for disc in lst_Inventory:             
                    for track in disc.cd_tracks:
                        # Adding cd_id to know which cd the track belongs to
                        strRow = str(disc.cd_id) + ',' + track.get_record()
                        file.write(strRow)
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            for cd in lst_Inventory:
                for row in lst:
                    if cd.cd_id == int(row[0]):
                        track = DC.Track(int(row[1]), row[2], row[3])
                        cd.cd_tracks.append(track)
